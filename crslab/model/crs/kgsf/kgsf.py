...
class KGSFModel(BaseModel):
    ...
    def recommend(self, batch, mode):
        ...
        Args:
            batch: Tuple containing context_entities, context_words, entities, movie.
            mode: Whether to train or test the model.

        Returns:
            In training mode, return the losses and scores. In testing mode,
              return the scores.
        """
        # Shapes
    ...
    def _decode_beam_search_with_kg(
        self,
        token_encoding,
        entity_reps,
        entity_emb_attn,
            ...
            for j in range(batch_size):
                ...
                for n in range(len(sequences[j])):
                    for k in range(beam):
                        ...
                        seq_tmp = torch.cat(
                            (
                                inputs[n][j].reshape(-1),
                                preds[n][j][0][k].reshape(-1),
                            )
                        )
                        candidate = [
                            seq_tmp,
    ...
    def converse(self, batch, mode):
        ...
        if mode != "test":
            logits, preds = self._decode_forced_with_kg(
                tokens_encoding,
                conv_entity_reps,
                conv_entity_emb,
                entity_padding_mask,
            ...
            logits, preds = self._decode_greedy_with_kg(
                tokens_encoding,
                conv_entity_reps,
                conv_entity_emb,
                entity_padding_mask,
