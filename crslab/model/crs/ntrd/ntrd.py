...
class NTRDModel(BaseModel):
    ...
    def _init_embeddings(self):
        ...
        nn.init.normal_(
            self.word_kg_embedding.weight, mean=0, std=self.kg_emb_dim**-0.5
        )
    ...
    def pretrain_infomax(self, batch):
        ...
        info_predict = F.linear(
            word_info_rep, entity_graph_representations, self.infomax_bias.bias
        )  # (bs, #entity)
    ...
    def recommend(self, batch, mode):
        ...
        rec_scores = F.linear(
            user_rep, entity_graph_representations, self.rec_bias.bias
        )  # (bs, #entity)
    ...
    def freeze_parameters(self):
        freeze_models = [
            self.word_kg_embedding,
            self.entity_encoder,
            self.entity_self_attn,
            self.word_encoder,
            self.word_self_attn,
            self.gate_layer,
            self.infomax_bias,
    ...
    def converse(self, batch, mode):
        ...
        if mode != "test":
            logits, preds, latent = self._decode_forced_with_kg(
                tokens_encoding,
                conv_entity_reps,
                conv_entity_emb,
                entity_padding_mask,
            ...
            assert torch.sum(all_movies != 0, dim=(0, 1)) == torch.sum(
                (response == 30000), dim=(0, 1)
            )  # 30000 means the idx of [ITEM]
            ...
            matching_logits = torch.masked_select(
                matching_logits_,
                masked_for_selection_token.unsqueeze(-1).expand_as(matching_logits_),
            ).view(-1, matching_logits_.shape[-1])
            ...
            logits, preds, latent = self._decode_greedy_with_kg(
                tokens_encoding,
                conv_entity_reps,
                conv_entity_emb,
                entity_padding_mask,
    ...
    def _decode_greedy_with_kg(
        ...
        for _ in range(self.response_truncate):
            ...
            copy_logits = self.copy_output(copy_latent) * self.copy_mask.unsqueeze(
                0
            ).unsqueeze(0)
    ...
    def _decode_forced_with_kg(
        ...
        dialog_latent, _ = self.conv_decoder(
            inputs, token_encoding, word_reps, word_mask, entity_reps, entity_mask
        )  # (bs, seq_len, dim)

        entity_latent = entity_emb_attn.unsqueeze(1).expand(-1, seq_len, -1)
        word_latent = word_emb_attn.unsqueeze(1).expand(-1, seq_len, -1)
        copy_latent = self.copy_norm(
            torch.cat((entity_latent, word_latent, dialog_latent), dim=-1)
        )  # (bs, seq_len, dim)
    ...
    def forward(self, batch, stage, mode):
        if len(self.gpu) >= 2:
            ...
            self.copy_mask = torch.as_tensor(
                np.load(os.path.join(self.dpath, "copy_mask.npy")).astype(bool),
