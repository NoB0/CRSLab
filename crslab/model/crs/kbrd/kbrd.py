    ...
    def encode_user(self, entity_lists, kg_embedding):
        ...
        for entity_list in entity_lists:
            if entity_list is None:
                user_repr_list.append(
                    torch.zeros(self.user_emb_dim, device=self.device)
                )
                continue
        ...
        for i in range(self.longest_label):
            ...
            with torch.no_grad():
                if i == 1:
                    ...
                    encoder_states = (
                        encoder_states[0].repeat(beam, 1, 1),
                        encoder_states[1].repeat(beam, 1, 1),
                    )

                scores, _ = self.decoder(
                    xs.reshape(len(sequences[0]) * bsz, -1), encoder_states
                )
                scores = scores[:, -1:, :]
                token_logits = F.linear(scores, self.token_embedding.weight)
                user_logits = self.user_proj_2(
