...
class SequenceCrossEntropyLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        if self.label_smoothing > 1.0:
            raise ValueError(
                "The param label_smoothing should be in the range of 0.0 to 1.0."
            )
        ...
        if self.label_smoothing > 0.0:
            ...
            one_hot_labels = torch.zeros_like(log_probs_flat).scatter_(
                -1, labels_flat, 1.0 - self.label_smoothing
            ...
            negative_log_likelihood_flat = -torch.gather(
                log_probs_flat, dim=1, index=labels_flat
            )  # (b_s * s_l, 1)
