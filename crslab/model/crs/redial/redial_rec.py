...
class ReDialRecModel(BaseModel):
    ...
    def build_model(self):
        ...
        if self.opt["autorec_f"] == "identity":
            ...
            raise ValueError(
                "Got invalid function name for f : {}".format(self.opt["autorec_f"])
            )
        ...
        if self.opt["autorec_g"] == "identity":
            ...
            raise ValueError(
                "Got invalid function name for g : {}".format(self.opt["autorec_g"])
            )

        self.encoder = nn.ModuleList(
            [
                nn.Linear(self.n_entity, self.layer_sizes[0])
                if i == 0
