...
class HRNN(nn.Module):
    ...
    def get_utterance_encoding(self, context, utterance_lengths):
        ...
        # reshape and reorder
        sorted_utterances = context.view(batch_size * max_conv_length, -1).index_select(
            0, sorted_idx
        )
        ...
        packed_utterances = pack_padded_sequence(
            embedded, sorted_lengths, batch_first=True
        )
        ...
        utterance_encoding = utterance_encoding.view(
            batch_size, max_conv_length, 2 * self.utterance_encoder_hidden_size
        )
    ...
    def forward(self, context, utterance_lengths, dialog_lengths):
        ...
        utterance_encoding = self.get_utterance_encoding(
            context, utterance_lengths
        )  # (bs, conv_len, 2 * utt_dim)
...
class SwitchingDecoder(nn.Module):
    def __init__(
        ...
        if context_size != hidden_size:
            raise ValueError(
                "The context size {} must match the hidden size {} in DecoderGRU".format(
    ...
    def forward(self, request, request_lengths, context_state):
        ...
        embedded_request = self.embedding(
            sorted_request
        )  # (batch_size, max_utterance_length, embed_dim)
        packed_request = pack_padded_sequence(
            embedded_request, sorted_lengths, batch_first=True
        )

        sorted_context_state = context_state.index_select(0, sorted_idx)
        h_0 = (
            sorted_context_state.unsqueeze(0)
            .expand(self.num_layers, batch_size, self.hidden_size)
            .contiguous()
        )  # require context_size == hidden_size

        sorted_vocab_state, _ = self.decoder(packed_request, h_0)
        sorted_vocab_state, _ = pad_packed_sequence(
            sorted_vocab_state, batch_first=True
        )  # (batch_size, max_request_length, decoder_hidden_size)

        _, max_request_length, decoder_hidden_size = sorted_vocab_state.shape
        pad_tensor = sorted_vocab_state.new_full(
            (
                batch_size,
                max_utterance_length - max_request_length,
                decoder_hidden_size,
            ),
            self.pad_token_idx,
        )
        sorted_vocab_state = torch.cat(
            (sorted_vocab_state, pad_tensor), dim=1
        )  # (batch_size, max_utterance_length, decoder_hidden_size)
        sorted_language_output = self.out(
            sorted_vocab_state
        )  # (batch_size, max_utterance_length, vocab_size)

        # expand context to each time step
        expanded_sorted_context_state = (
            sorted_context_state.unsqueeze(1)
            .expand(batch_size, max_utterance_length, self.context_size)
            .contiguous()
        )  # (batch_size, max_utterance_length, context_size)
        # compute switch
        switch_input = torch.cat(
            (expanded_sorted_context_state, sorted_vocab_state), dim=2
        )  # (batch_size, max_utterance_length, context_size + decoder_hidden_size)
        ...
        output = sorted_output.index_select(
            0, rev_idx
