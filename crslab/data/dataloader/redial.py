...
class ReDialDataLoader(BaseDataLoader):
    ...
    def rec_batchify(self, batch):
        ...
        return {
            ...
            "item": torch.tensor(batch_item, dtype=torch.long),
        }
    ...
    def conv_process_fn(self):
        ...
        for conversation in tqdm(self.dataset):
            if conversation["role"] != "Recommender":
                continue
            context_tokens = [
                truncate(utterance, self.utterance_truncate, truncate_tail=True)
                for utterance in conversation["context_tokens"]
            ]
            context_tokens = truncate(
                context_tokens, self.conversation_truncate, truncate_tail=True
            )
            context_length = len(context_tokens)
            utterance_lengths = [len(utterance) for utterance in context_tokens]
            request = context_tokens[-1]
            response = truncate(
                conversation["response"], self.utterance_truncate, truncate_tail=True
            )
            dataset.append(
                {
    ...
    def conv_batchify(self, batch):
        ...
        for conversation in batch:
            ...
            batch_utterance_lengths.append(
                conversation["utterance_lengths"]
