...
class TGReDialDataLoader(BaseDataLoader):
    ...
    def _process_rec_context(self, context_tokens):
        ...
        compat_context = truncate(
            merge_utt(compact_context), self.context_truncate - 2, truncate_tail=False
        )
        compat_context = add_start_end_token_idx(
    ...
    def _process_history(self, context_items, item_id=None):
        input_ids = truncate(
            context_items, max_length=self.item_truncate, truncate_tail=False
        )
    ...
    def rec_batchify(self, batch):
        ...
        for conv_dict in batch:
            ...
            if "interaction_history" in conv_dict:
                context_items = (
                    conv_dict["interaction_history"] + conv_dict["context_items"]
                )
            else:
        ...
        batch_context = padded_tensor(
            batch_context, self.pad_token_idx, max_len=self.context_truncate
        )
    ...
    def rec_interact(self, data):
        ...
        context = padded_tensor(
            context, self.pad_token_idx, max_len=self.context_truncate
        )
    ...
    def conv_batchify(self, batch):
        ...
        for conv_dict in batch:
            context_tokens = [
                utter + [self.conv_bos_id] for utter in conv_dict["context_tokens"]
            ]
            context_tokens[-1] = context_tokens[-1][:-1]
            batch_context_tokens.append(
                truncate(
                    merge_utt(context_tokens),
            ...
            batch_response.append(
                add_start_end_token_idx(
                    truncate(
                        conv_dict["response"], max_length=self.response_truncate - 2
                    ),
            ...
            batch_context_words.append(
                truncate(
                    conv_dict["context_words"], self.word_truncate, truncate_tail=False
                )
            )
            ...
            if "target" in conv_dict:
                ...
                enhanced_topic = merge_utt(
                    enhanced_topic, self.word_split_idx, False, self.sent_split_idx
                )
            ...
            if "items" in conv_dict:
                ...
                enhanced_movie = truncate(
                    merge_utt(enhanced_movie, self.word_split_idx, self.sent_split_idx),
                    self.item_truncate,
                    truncate_tail=False,
                )
        ...
        batch_context_tokens = padded_tensor(
            items=batch_context_tokens,
            pad_idx=self.pad_token_idx,
            max_len=self.context_truncate,
            pad_tail=False,
        )
        batch_response = padded_tensor(
            batch_response,
            pad_idx=self.pad_token_idx,
    ...
    def conv_interact(self, data):
        ...
        context_words = [
            truncate(data["context_words"], self.word_truncate, truncate_tail=False)
        ]
    ...
    def policy_batchify(self, batch):
        ...
        for conv_dict in batch:
            ...
            final_topic = merge_utt(
                final_topic, self.word_split_idx, False, self.sep_id
            )

            context = conv_dict["context_tokens"]
            context = merge_utt(context, self.sent_split_idx, False, self.sep_id)
            context += final_topic
            context = add_start_end_token_idx(
                truncate(
                    context, max_length=self.context_truncate - 1, truncate_tail=False
                ),
            ...
            context_policy = add_start_end_token_idx(
                context_policy, start_token_idx=self.cls_id, end_token_idx=self.sep_id
            )
        ...
        batch_context_policy = padded_tensor(
            batch_context_policy, pad_idx=self.pad_token_idx, pad_tail=True
        )
        batch_context_policy_mask = (batch_context_policy != 0).long()
        batch_user_profile = padded_tensor(
            batch_user_profile, pad_idx=self.pad_token_idx, pad_tail=True
        )
        ...
        return (
            ...
            batch_cotnext_mask,
            batch_context_policy,
            batch_context_policy_mask,
            batch_user_profile,
            batch_user_profile_mask,
            batch_target,
        )
