...
class TGReDialSystem(BaseSystem):
    ...
    def __init__(
        ...
        """
        super(TGReDialSystem, self).__init__(
            opt,
            train_dataloader,
            valid_dataloader,
            test_dataloader,
        ...
        if hasattr(self, "conv_model"):
            ...
            if (
                self.conv_optim_opt.get("lr_scheduler", None)
                and "Transformers" in self.conv_optim_opt["lr_scheduler"]["name"]
            ):
                batch_num = 0
                for _ in self.train_dataloader["conv"].get_conv_data(
                    batch_size=self.conv_batch_size, shuffle=False
                ):
                    batch_num += 1
                conv_training_steps = self.conv_epoch * floor(
                    batch_num / self.conv_optim_opt.get("update_freq", 1)
                )
                self.conv_optim_opt["lr_scheduler"][
                    "training_steps"
                ] = conv_training_steps
        ...
        if stage == "policy":
            ...
            if isinstance(policy_loss, torch.Tensor):
                ...
                self.evaluator.optim_metrics.add(
                    "policy_loss", AverageMetric(policy_loss)
                )
        elif stage == "rec":
            if mode == "train":
    ...
    def train_recommender(self):
        ...
        other_param = [
            name_param
            for name_param in self.rec_model.named_parameters()
        ...
        for epoch in range(self.rec_epoch):
            ...
            for batch in self.train_dataloader["rec"].get_rec_data(
    ...
    def train_policy(self):
        ...
        params = [
            {
                "params": [
                    p for n, p in policy_params if not any(nd in n for nd in no_decay)
                ],
                "weight_decay": self.policy_optim_opt["weight_decay"],
            },
            {
                "params": [
                    p for n, p in policy_params if any(nd in n for nd in no_decay)
                ],
            },
        ]
    ...
    def convert_to_id(self, text, stage):
        ...
        token_ids = [
            self.vocab[stage]["tok2ind"].get(token, self.vocab[stage]["unk"])
            for token in tokens
        ]
        entity_ids = [
            self.vocab[stage]["entity2id"][entity]
            for entity in entities
            if entity in self.vocab[stage]["entity2id"]
        ]
        movie_ids = [
            entity_id for entity_id in entity_ids if entity_id in self.item_ids
        ]
        word_ids = [
            self.vocab[stage]["word2id"][word]
            for word in words
            if word in self.vocab[stage]["word2id"]
        ]
