...
class InvSqrtLR(LRScheduler):
    ...
    def __init__(
        self, optimizer, invsqrt_lr_decay_gamma=-1, last_epoch=-1, warmup_steps=0
    ):
        """
        ...
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, self._invsqrt_lr, last_epoch
        )
...
class CosineAnnealingWarmRestartsLR(LRScheduler):
    def __init__(
        self, optimizer, T_0, T_mult=1, eta_min=0, last_epoch=-1, warmup_steps=0
    ):
...
class TransformersLinearLR(LRScheduler):
    ...
    def _linear_lr(self, step):
        return max(
            0.0, float(self.training_steps - step) / float(max(1, self.training_steps))
        )
...
class TransformersCosineLR(LRScheduler):
    def __init__(
        ...
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, self._cosine_lr, last_epoch
        )
...
class TransformersCosineWithHardRestartsLR(LRScheduler):
    def __init__(
        ...
        super(TransformersCosineWithHardRestartsLR, self).__init__(
            optimizer, warmup_steps
        )
        self.training_steps = training_steps - warmup_steps
        self.num_cycles = num_cycles
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, self._cosine_with_hard_restarts_lr, last_epoch
        )
    ...
    def _cosine_with_hard_restarts_lr(self, step):
        ...
        return max(
            0.0,
            0.5
            * (1.0 + math.cos(math.pi * ((float(self.num_cycles) * progress) % 1.0))),
        )
...
class TransformersPolynomialDecayLR(LRScheduler):
    def __init__(
        ...
        assert (
            self.lr_init > lr_end
        ), f"lr_end ({lr_end}) must be be smaller than initial lr ({self.lr_init})"
        self.power = power
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, self._polynomial_decay_lr, last_epoch
        )
