...
class NTRDSystem(BaseSystem):
    ...
    def __init__(
        ...
        super(NTRDSystem, self).__init__(
            opt,
            train_dataloader,
            valid_dataloader,
            test_dataloader,
    ...
    def conv_evaluate(self, prediction, movie_prediction, response, movie_response):
        ...
        if movie_prediction != None:
            ...
            movie_prediction = torch.masked_select(
                movie_prediction, (movie_prediction != 0)
            )
        ...
        for p, r in zip(prediction, response):
            p_str = ind2txt_with_slots(
                p, movie_prediction, self.ind2tok, self.end_token_idx
            )
            p_str = p_str[1:]
            r_str = ind2txt_with_slots(
                r, movie_response, self.ind2tok, self.end_token_idx
            )
            ...
            if mode != "test":
                ...
                if mode == "train":
                    ...
                    self.evaluator.optim_metrics.add(
                        "gen_total_loss", AverageMetric(loss)
                    )
                ...
                self.evaluator.optim_metrics.add(
                    "sel_loss", AverageMetric(selection_loss)
                )
