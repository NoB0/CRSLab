...
class BaseSystem(ABC):
    ...
    def __init__(
        ...
        # model
        if "model" in opt:
            self.model = get_model(opt, opt["model"], self.device, vocab, side_data).to(
                self.device
            )
        else:
            if "rec_model" in opt:
                self.rec_model = get_model(
                    opt, opt["rec_model"], self.device, vocab["rec"], side_data["rec"]
                ).to(self.device)
            if "conv_model" in opt:
                self.conv_model = get_model(
                    opt,
                    opt["conv_model"],
                    self.device,
                    vocab["conv"],
        ...
        if not interact:
            self.evaluator = get_evaluator(
                opt.get("evaluator", "standard"), opt["dataset"], tensorboard
            )
    ...
    def build_lr_scheduler(self):
        ...
        if self.optim_opt.get("lr_scheduler", None):
            ...
            self.scheduler = lr_scheduler_class[lr_scheduler](
                self.optimizer, **lr_scheduler_opt
            )
    ...
    def early_stop(self, metric):
        ...
        if (
            ...
            or metric * self.stop_mode > self.best_valid * self.stop_mode
        ):
    ...
    def update_context(
        self, stage, token_ids=None, entity_ids=None, item_ids=None, word_ids=None
    ):
